...
var renderFile = module.exports = function(file, options, fn){
  ...
  ejs.renderFile(file, options, function(err, html) {
    ...
    // set a default layout file for the template or the app
    ...
    if (layout) {
      ...
      var engine = options.settings['template engine'] || 'ejs',
          desiredExt = '.'+engine;
      ...
      if (layout.length > 0 && layout[0] === path.sep) {
        // if layout is an absolute path, find it relative to template options:
...
/**
 * Resolve partial object name from the template path.
 *
 * Examples:
 *
 ...
 * Options:
 *
 *   - `cache` store the resolved path for the template, to avoid disk I/O
 *
...
function lookup(root, partial, options){
  ...
  //   it as well as relative to the current template
...
/**
 * Render `template` partial with the given `options`. Optionally a
 * callback `fn(err, str)` may be passed instead of writing to
 * the socket.
 *
 * Options:
 *
 *   - `object` Single object with name derived from the template (unless `as` is present)
 *
 *   - `as` Variable name for each `collection` value, defaults to the template name.
 *     * as: 'something' will add the `something` local variable
 *     * as: this will use the collection value as the template context
 *     * as: global will merge the collection value's properties with `locals`
 *
 *   - `collection` Array of objects, the name is derived from the template name itself.
...
function partial(view, options){
  ...
  // extract object name from template
  name = options.as || resolveObjectName(view);

  // find template, relative to this filename
...
/**
 * Apply the given `template` as the layout for the current template,
 * using the current options/locals. The current template will be
 * supplied to the given `template` as `body`, along with any `blocks`
